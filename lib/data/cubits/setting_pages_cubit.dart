...
class SettingsPagesFetchFailure extends SettingsPagesState {
  SettingsPagesFetchFailure(this.error);
  final Object error;
}
...
class SettingsPagesCubit extends Cubit<SettingsPagesState> {
  ...
  void fetchSettingsPages(String title, {bool? forceRefresh}) async {
    ...
    if (forceRefresh == true) {
      fetchSettingsPagesFromDb(title)
          .then((value) {
            emit(SettingsPagesFetchSuccess(data: value ?? ""));
          })
          .catchError((e, stack) {
            emit(SettingsPagesFetchFailure(e));
          });
    } else {
      if (state is! SettingsPagesFetchSuccess) {
        fetchSettingsPagesFromDb(title)
  ...
  Future<String?> fetchSettingsPagesFromDb(String title) async {
    try {
      ...
      if (!response[Api.error]) {
        ...
        if (title == Api.refundPolicy) {
          settingsPagesData = data['refund_policy'];
        }
      } else {
