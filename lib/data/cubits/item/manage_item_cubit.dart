...
class ManageItemCubit extends Cubit<ManageItemState> {
  ...
  void manage(ManageItemType type, Map<String, dynamic> data) async {
    try {
      emit(ManageItemInProgress());

      if (type == ManageItemType.add) {
        ItemModel itemModel = await _itemRepository.createItem(data);
        emit(ManageItemSuccess(itemModel, type));
      } else if (type == ManageItemType.edit) {
        ItemModel itemModel = await _itemRepository.editItem(data);
        emit(ManageItemSuccess(itemModel, type));
      }
    } catch (e) {
