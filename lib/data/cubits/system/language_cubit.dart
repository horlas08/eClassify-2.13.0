...
class LanguageCubit extends Cubit<LanguageState> {
  ...
  void loadCurrentLanguage() {
    ...
    if (language != null) {
      ...
      AppSession.setCurrentLanguage(languageCode, countryCode);
  ...
  void changeLanguages(dynamic map) {
    final languageCode = map['code'].toString();
    final countryCode = map['country_code'].toString();
    AppSession.setCurrentLanguage(languageCode, countryCode);

    // Long Messages for items
    timeago.setLocaleMessages(
      AppSession.currentLocale,
      TimeagoMessages.getMessages(AppSession.currentLanguageCode),
    );
    // Short Messages for chats
    timeago.setLocaleMessages(
      '${AppSession.currentLocale}_short',
      TimeagoMessages.getMessages('${AppSession.currentLanguageCode}_short'),
    );
