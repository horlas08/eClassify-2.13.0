...
class BlogsScreen extends StatefulWidget {
  const BlogsScreen({required this.blogId, super.key});
  ...
  static Route route(RouteSettings settings) {
    return MaterialPageRoute(
      builder: (context) {
        return BlogsScreen(blogId: settings.arguments as int?);
      },
    );
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: RefreshIndicator(
          ...
          child: BlocConsumer<FetchBlogsCubit, FetchBlogsState>(
            listener: (context, state) {
              if (state is FetchBlogsSuccess && widget.blogId != null) {
                final notificationBlog = state.blogModel.firstWhereOrNull(
                  (element) => element.id == widget.blogId,
                );
                if (notificationBlog != null) {
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    Navigator.of(context).pushNamed(
                      Routes.blogDetailsScreenRoute,
                      arguments: {'blog': notificationBlog},
                    );
                  });
                }
              }
